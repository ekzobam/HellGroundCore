    ...
    if (load_command_table)
    {
        ...
        if (result)
        {
            do
            {
                ...
                SetDataForCommandInTable(commandTableCache, name.c_str(), fields[1].GetUInt16(), fields[2].GetString(), name, fields[3].GetUInt32());
...
bool ChatHandler::SetDataForCommandInTable(ChatCommand *table, const char* text, uint32 security, std::string const& help, std::string const& fullcommand, uint32 groupAllowMask = 0)
{
    std::string cmd = "";
    ...
    for (uint32 i = 0; table[i].Name != NULL; i++)
    {
        ...
        if (table[i].ChildCommands != NULL)
        {
            if (SetDataForCommandInTable(table[i].ChildCommands, text, security, help, fullcommand, groupAllowMask))
                return true;
            else if (*text)
                return false;
