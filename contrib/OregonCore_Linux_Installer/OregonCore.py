...
def DownloadSource():
    ...
    time.sleep(2)
    cmd( "sudo apt-get update && apt-get upgrade && sudo apt-get install screen git cmake make gcc g++ clang cppcheck libmariadbclient-dev libreadline-dev libncurses-dev mariadb-server libace-6.* libace-dev  build-essential binutils-dev libiberty-dev libbz2-dev openssl libssl-dev zlib1g-dev libtool mariadb-client zip unrar unzip doxygen default-libmysqlclient-dev -y")
    cmd("clear")
